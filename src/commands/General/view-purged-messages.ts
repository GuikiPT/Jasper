...
export class ViewPurgedMessagesCommand extends Command {
	...
	public override registerApplicationCommands(registry: Command.Registry) {
		...
		registry.registerChatInputCommand((builder) =>
			builder
				.setName('view-gearbot-logs')
				.setDescription('View purged messages from a GearBot archive in a visual format')
				.addAttachmentOption((option) =>
					option
						.setName('logfile')
						.setDescription('The GearBot log file (.txt)')
						.setRequired(false)
				)
				.addStringOption((option) =>
					option
						.setName('message-link')
						.setDescription('Link to a message containing the log file')
						.setRequired(false)
				)
				.setDMPermission(false)
		);
	...
	public override async contextMenuRun(interaction: Command.ContextMenuCommandInteraction): Promise<void> {
		...
		try {
			...
			await this.processLogFile(interaction, txtAttachment.url);
		} catch (error) {
			this.container.logger.error('[View Purged Messages] Error processing context menu:', error);
			await interaction.editReply({
	...
	public override async chatInputRun(interaction: Command.ChatInputCommandInteraction): Promise<void> {
		...
		try {
			...
			if (!attachment && !messageLink) {
				await interaction.editReply({
					content: '❌ Please provide either a log file attachment or a message link.'
				});
				return;
			}
			...
			if (attachment) {
				...
				if (!attachment.contentType?.includes('text') && !attachment.name?.endsWith('.txt')) {
					await interaction.editReply({
						content: '❌ Please provide a valid text file (.txt)'
					});
					return;
				}

				await this.processLogFile(interaction, attachment.url);
				return;
			}

			// Handle message link
			if (messageLink) {
				// Parse message link format: https://discord.com/channels/{guild_id}/{channel_id}/{message_id}
				...
				try {
					...
					if (!channel?.isTextBased()) {
						await interaction.editReply({
							content: '❌ Could not access the channel from the message link.'
						});
						return;
					}
					...
					if (!txtAttachment) {
						await interaction.editReply({
							content: '❌ No text file found in the linked message.'
						});
						return;
					}
					...
					this.container.logger.error('[View Purged Messages] Error fetching message:', error);
					await interaction.editReply({
						content: '❌ Could not fetch the message. Make sure the bot has access to the channel and the message exists.'
					});
				}
			}
		} catch (error) {
			this.container.logger.error('[View Purged Messages] Error processing slash command:', error);
			await interaction.editReply({
				content: '❌ An error occurred while processing the archive. Please make sure it\'s a valid purged messages file.'
			});
		}
	...
	private async processLogFile(interaction: Command.ChatInputCommandInteraction | Command.ContextMenuCommandInteraction, fileUrl: string): Promise<void> {
