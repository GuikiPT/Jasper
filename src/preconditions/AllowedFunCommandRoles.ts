// AllowedFunCommandRoles module within preconditions
...
// Precondition enforcing AllowedFunCommandRoles access rules.
export class AllowedFunCommandRolesPrecondition extends AllFlowsPrecondition {
	...
	private createErrorMessage(allowedRoles: string[]): string {
		if (allowedRoles.length === 0) {
			return 'Fun commands may only be used by users with "Allowed Fun Command Roles". No roles are currently configured.';
		}
	...
	private async fetchAllowedRoles(guildId: string) {
		...
		if (!service) {
			this.container.logger.error('[AllowedFunCommandRoles] Role settings service is unavailable');
			return [];
		}

		try {
			return await service.listBucket(guildId, 'allowedFunCommandRoles');
		} catch (error) {
