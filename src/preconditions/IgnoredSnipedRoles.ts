...
export class IgnoredSnipedRolesPrecondition extends AllFlowsPrecondition {
	...
	private async checkMemberAccess(guildId: string | null, member: GuildMember | APIInteractionGuildMember | null, silentOnFail: boolean) {
		try {
			...
			if (allowedRoles.length === 0) {
				this.logDenial('no-config', { guildId, memberId: this.resolveMemberId(member), silentOnFail });
				return this.error({
					message: this.createErrorMessage([]),
					context: silentOnFail ? { silent: true } : {}
				});
	...
	private logGrant(details: { guildId: string | null; memberId: string; allowedRoles: readonly string[]; silentOnFail: boolean }) {
		this.container.logger.debug('[IgnoredSnipedRoles] Access granted', {
	...
	private logDenial(
		...
		this.container.logger[level]('[IgnoredSnipedRoles] Access denied', {
			reason,
			guildId: details.guildId ?? 'unknown',
			memberId: details.memberId,
			allowedRoles: details.allowedRoles ?? [],
