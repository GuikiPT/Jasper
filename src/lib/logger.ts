...
function normalizeError(error: unknown) {
	if (error instanceof Error) {
		...
		if ('cause' in error && error.cause !== undefined) normalized.errorCause = sanitizeValue((error as Error & { cause?: unknown }).cause);
...
function formatForFile(value: unknown): string {
	if (value instanceof Error) {
		return JSON.stringify({
			errorName: stripAnsi(value.name),
			errorMessage: stripAnsi(value.message),
			errorCause: 'cause' in value && value.cause !== undefined ? sanitizeValue((value as Error & { cause?: unknown }).cause) : undefined,
			errorStack: value.stack ? stripAnsi(value.stack) : undefined
		});
	}
...
export class JasperLogger extends BaseLogger {
	...
	public constructor(options: JasperLoggerOptions = {}) {
		const { consoleLevel = options.level ?? LogLevel.Info, fileLevel = LogLevel.Info, ...rest } = options;
	...
	private async writeToDisk(level: LogLevel, values: readonly unknown[]) {
		...
		const levelSlug = this.levelToSlug(level);
		const levelDirectory = join(rootDir, 'logs', 'levels', levelSlug, year, month);
		const levelFilePath = join(levelDirectory, `${day}.log`);

		try {
			const entry = `${now.toISOString()} ${getLevelLabel(level)} ${this.formatValues(values)}\n`;
			await this.appendEntry(directory, filePath, entry);
			await this.appendEntry(levelDirectory, levelFilePath, entry);
		} catch (error) {
	...
	/**
	 * Normalizes level into a filesystem-friendly folder name
	 */
	private levelToSlug(level: LogLevel) {
		return getLevelLabel(level).toLowerCase();
	}

	/**
	 * Ensures target directory exists and appends an entry to the file
	 */
	private async appendEntry(directory: string, filePath: string, entry: string) {
