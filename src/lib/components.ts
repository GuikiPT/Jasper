// Component builder utilities - helpers for creating Discord Components v2
import {
	MessageFlags,
	ContainerBuilder,
	TextDisplayBuilder,
...
/**
 * Truncates title text (removes markdown formatting if needed)
 */
function truncateTitle(title: string): string {
	// Remove markdown headers first
	const cleanTitle = title.replace(/^#{1,6}\s*/, '');
	return truncateText(cleanTitle, 100);
}
...
export function createErrorTextComponent(content: string) {
	return new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(truncateText(content, 2000)));
}
...
/**
 * Creates a component-based reply for public or ephemeral messages
 */
export function replyWithComponent(interaction: CommandInteraction, content: string, ephemeral: boolean = false) {
	const components = [createErrorTextComponent(content)];
...
/**
 * Creates a component with title, summary, details list, and optional thumbnail
 */
...
export function createListComponent(title: string, items: string[], emptyMessage: string = 'No items found.', forceNewlines: boolean = false) {
	...
	if (items.length === 0) {
		...
		const useNewlines = forceNewlines || averageItemLength > 50;

		const separator = useNewlines ? '\n' : ', ';

		// Smart pagination: Fit as many items as possible within limit
		...
		if (fittingItems.length < items.length) {
			...
			if (currentLength + moreText.length <= maxLength) {
				...
				// Remove last item to make room for "more" text
...
export function createMultiSectionComponent(
	...
	// Limit to 7 sections to accommodate all role types
	...
	for (let index = 0; index < limitedSections.length; index += 1) {
		...
		// Only add separator between sections (not after last section)
...
/**
 * Creates a paginated component for large lists (without buttons, shows first page only)
 */
export function createPaginatedListComponent(title: string, items: string[], emptyMessage: string = 'No items found.', itemsPerPage: number = 10) {
	...
	// Calculate pagination
	const totalPages = Math.ceil(items.length / itemsPerPage);
	const currentPage = 1; // Always show first page
...
export async function respondWithComponent(
	...
	try {
		...
		if (context.message) {
			// For message commands, fall back to text content extraction
		...
		Logger.error('Failed to respond with component', error, {
			hasInteraction: Boolean(context.interaction),
			hasMessage: Boolean(context.message),
			deferred: Boolean(context.interaction?.deferred),
			replied: Boolean(context.interaction?.replied)
		});

		if (context.interaction) {
			const fallbackContent = 'There was an error sending that response. Please try again.';
			...
			try {
				...
				Logger.error('Failed to send fallback interaction response', fallbackError);
			}
		}

		if (context.message) {
			try {
				return context.message.reply('There was an error sending that response.');
			} catch (fallbackError) {
				Logger.error('Failed to send fallback message response', fallbackError);
			}
		}
 ...
 * Note: This is a simplified extraction for message command compatibility
 */
function extractContentFromComponent(_component: ContainerBuilder): string {
	// Simplified extraction - message commands don't support full component rendering
